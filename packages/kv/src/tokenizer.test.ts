...
describe("Individual consumption", () => {

    test("Consume Unquoted string", () => {
        let text = "hello_this_is_an_unquoted_string";
        const len = consumeStringUnquoted(text, 0);
        expect(len).toBe(text.length);
        
        text = "key value";
        expect(consumeStringUnquoted(text, 0)).toBe(3);
        expect(consumeStringUnquoted(text, 4)).toBe(5);
    });
